...
use rmb_display::{ContentClass, FsmConfig, RefreshController, Waveform, WaveformMode};
use rmb_engine::{DomPosition, LinkInfo};
use rmb_hal::{mxcfb, DisplayBackend, UpdateMarker, UpdateRequest};
#[cfg(not(feature = "device"))]
use rmb_hal::MemoryBackend;
...
use rmb_types::{Device, Orientation, PanelFormat, Rect};

/// Render a horizontal grey ramp through the mono pipeline, blit it, and submit a refresh.
/// Backend-agnostic: drives the real panel on-device, or the in-memory buffer on host.
fn render_grey_ramp<B: DisplayBackend>(backend: &mut B, fmt: PanelFormat, region: Rect) -> UpdateMarker {
    ...
    pipe.convert(&src, Rect::new(0, 0, rw, rh), &mut dst);
    backend.blit(region, &dst.bytes);
    let marker = backend.submit(&UpdateRequest {
        region,
        waveform: Waveform::Mono(WaveformMode::Gc16),
        full: false,
    });
    backend.wait(marker);
    marker
...
fn main() {
    ...
    let region = Rect::new(0, 920, fb_w, 32); // a band near vertical centre

    // On-device (`--features device`) this drives the real panel via rm2fb; otherwise it
    // renders into a host in-memory framebuffer we can inspect.
    #[cfg(feature = "device")]
    let mut backend = rmb_hal::rm2fb::Rm2Backend::new();
    #[cfg(not(feature = "device"))]
    let mut backend = MemoryBackend::new(fb_w, fb_h, fmt);

    let marker = render_grey_ramp(&mut backend, fmt, region);
    println!("\nmono render (grey ramp, ordered dither):");
    println!("  region        : {region:?}");
    println!("  submit marker : {marker:?}");
    #[cfg(not(feature = "device"))]
    {
        let mid = backend.pixel(fb_w / 2, region.y as u32 + 16);
