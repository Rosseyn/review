...
including an **end-to-end mono render path** (RGBA → dither → blit → submit) exercised on a
host in-memory backend. Real device I/O (rm2fb transport, the WPE engine) are the next
device-facing steps.

```
cargo test --workspace     # 29 tests across the implemented crates
cargo run -p rmbrowser      # RM2 smoke test: mono render + FSM + nav + bookmarks, no device
```
...
| `rmb-hal` | `DisplayBackend` trait + **MemoryBackend** + **mxcfb waveform→mode-id** mapping; rm2fb/mxcfb device backends (PLAN §4) | host backend + protocol done + tested; device backends scaffold |
